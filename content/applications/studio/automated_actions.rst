...
#. Click :icon:`fa-arrow-right` :guilabel:`Scheduled action` to open the :guilabel:`Automation
   ...
   .. tip::
      By default, this scheduled action is set to :guilabel:`Execute Every` 4 hours, i.e., to check
      for execution date times that have been reached since the last run of the scheduled
      action, and to execute related actions, if relevant.

      This frequency is generally sufficient for delays such as 3 months after the order date or 7
      days after the last update. The scheduled action runs more frequently if any time-based
      trigger has a delay of less than the equivalent of 40 hours, e.g., 1 hour before the event
      start date and time, to allow the delay to be respected more closely.
...
#. Click :guilabel:`Run manually` at the top left.
   ...
   .. warning::
      ...
      automation rule, which is a key element of the :ref:`execution logic of the rule
